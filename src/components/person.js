...
class Person extends Component {
  render () {
    ...
    if (!this.props.search) {
      ...
      return (
        <p className='person'>
          <a onClick={this.props.nominate} data-key={this.props.name.key} dangerouslySetInnerHTML={{__html: fullName}} />
        </p>
      )
    } else { // If no search results in list, add new component for new name
      return (
